...
def m1(TS_):
    ...
    #  ══════════════════ 남쪽 통창 y27 ══════════════════
    #  기둥 (6,20)-(6,21) — 화이트보드 x12~16 에서 연상 자리 (5,24) 로 가는
    #  시선 위. 그 선은 y21 에서 x6 을 지난다. 뒷줄 책상(y22)과 안 겹치게 한 칸 위.
    ...
    m.place(E, "pillar", 6, 20)

    # ---- NPC (지정석 = 캐릭터 묘사)
    SEATS = {
        "player":  (15, 16),                                   # 정중앙
        "younger": (22, 12),                                   # 앞줄 · 문2(x23) 옆
        "older":   (5, 24),                                    # 뒷줄 · 기둥(6,20) 뒤
    ...
    # ---- 계단 (4F ↔ 2F 직결 · 3F는 만들지 않는다)
    m.place(E, "stairs_down", 31, 24)
    m.portal("m2_basecamp_2f", 31, 24, 20, 5, name="stairs_to_2f")
    m.portal("m2_basecamp_2f", 32, 24, 21, 5, name="stairs_to_2f")
...
def m2(TS_):
    ...
    m.portal("m1_basecamp_4f", 21, 4, 32, 26, name="stairs_to_4f")

...
STAIRS — 계단은 **복도에 놓는다. 층마다 다른 자리다.**
    올라가는 계단은 네 층 모두 위쪽 복도 (20,3) 이고 앞칸이 (20,5) 다.
    위쪽 복도는 y2~5 로 네 층에 똑같이 나 있어서 좌표를 공유할 수 있다.

    내려가는 계단은 그럴 수가 없다. 교육장 밖 **세로 복도가 층마다 다른
    x 에 난다** — 방 크기가 층마다 다르기 때문이다.
        4F  x31~34   교육장(x2~29) ↔ 코칭실·라운지(x36~42) 사이
        2F  x27~29   오픈데스크(x2~25) ↔ 회의실 5~7(x31~41) 사이
        1F  x23~25   로비(x2~21) ↔ 운영사무실(x27~34) 사이 · y16 아래로 넓어진다
    네 층에 같은 좌표를 쓰면 어느 층에서는 방 안이나 벽 속에 박힌다.
    그래서 층마다 제 복도의 아래쪽(y24~25)에 놓는다 — 맨 아랫줄(y28)은
    한 칸짜리라 위에서 내려설 자리가 없어서 거기까지는 안 내린다.

        4F  (32,24)  앞칸 (32,23)
        2F  (27,24)  앞칸 (27,23)
        1F  (23,24)  앞칸 (23,23)
    B1 은 맨 아래층이라 올라가는 계단만 있다.

    내려가면 아래층의 「올라가는 계단 앞」 (20,5) 에, 올라가면 위층의
    「내려가는 계단 앞」 에 나온다 — 되돌아가려면 뒤로 한 발이면 된다.
    **아래층의 올라가는 계단이 spawn 으로 가리키는 좌표가 곧 위층
    내려가는 계단의 앞칸이다.** 한쪽만 옮기면 엉뚱한 데로 튀어나온다.
"""
...
def m1(TS_):
    ...
    # ---- 계단 — **내려가는 계단은 층마다 자리가 다르다** (STAIRS 주석 참고)
    #   올라가는 계단  (20,3)  · 위쪽 복도 · 아래에서만 · 앞칸 (20,5)
    #   내려가는 계단  (32,24) · 교육장 동쪽 세로 복도 · 위에서만 · 앞칸 (32,23)
    # 4F 의 복도는 교육장(x2~29)과 코칭실·라운지(x36~42) 사이 x31~34 다.
    # 4F 는 꼭대기라 내려가는 계단만 있다 (3F 는 만들지 않는다)
    m.place(E, "stairs_down", 32, 24)
    m.portal("m2_basecamp_2f", 32, 24, 20, 5, name="stairs_to_2f")
    m.portal("m2_basecamp_2f", 33, 24, 20, 5, name="stairs_to_2f")
...
def m2(TS_):
    ...
    # 휴지통(26,5)은 뺐다 — 소파(27,3)의 왼쪽 아래 대각에 딱 걸려서,
    # 복도 y5 로 지나가려 하면 여기서 막혔다. 라운지 집기는 소파·자판기·
    # 정수기로 충분하고, 이 한 칸이 통로다.
    ...
    # ---- 계단 — **내려가는 계단은 층마다 자리가 다르다** (STAIRS 주석 참고)
    #   올라가는 계단  (20,3)  · 위쪽 복도 · 아래에서만 · 앞칸 (20,5)
    #     → 4F 의 내려가는 계단 앞칸 (32,23) 으로 올려보낸다
    #   내려가는 계단  (27,24) · 오픈데스크 동쪽 세로 복도 · 위에서만 · 앞칸 (27,23)
    # 2F 의 복도는 오픈데스크(x2~25)와 회의실 5~7(x31~41) 사이 x27~29 다.
    m.place(E, "stairs_up", 20, 3)
    m.portal("m1_basecamp_4f", 20, 4, 32, 23, name="stairs_to_4f")
    m.portal("m1_basecamp_4f", 21, 4, 32, 23, name="stairs_to_4f")
    m.place(E, "stairs_down", 27, 24)
    m.portal("m3_basecamp_1f", 27, 24, 20, 5, name="stairs_to_1f")
    m.portal("m3_basecamp_1f", 28, 24, 20, 5, name="stairs_to_1f")
...
def m3(TS_):
    ...
    # ---- 계단 — **내려가는 계단은 층마다 자리가 다르다** (STAIRS 주석 참고)
    #   올라가는 계단  (20,3)  · 위쪽 복도 · 아래에서만 · 앞칸 (20,5)
    #     → 2F 의 내려가는 계단 앞칸 (27,23) 으로 올려보낸다
    #   내려가는 계단  (23,24) · 로비 동쪽 세로 복도 · 위에서만 · 앞칸 (23,23)
    # 1F 의 복도는 로비(x2~21)와 운영사무실(x27~34) 사이 x23~25 인데,
    # 사무실·택배보관소가 y15 에서 끝나 그 아래로는 오른쪽이 통째로 트인다.
    m.place(E, "stairs_up", 20, 3)
    m.portal("m2_basecamp_2f", 20, 4, 27, 23, name="stairs_to_2f")
    m.portal("m2_basecamp_2f", 21, 4, 27, 23, name="stairs_to_2f")
    m.place(E, "stairs_down", 23, 24)                # B1 (정글스테이지 · 카페테리아 · 편의점)
    m.portal("m4_basecamp_b1", 23, 24, 20, 5, name="stairs_to_b1")
    m.portal("m4_basecamp_b1", 24, 24, 20, 5, name="stairs_to_b1")
...
def m4(TS_):
    ...
    # ---- 계단 — **내려가는 계단은 층마다 자리가 다르다** (STAIRS 주석 참고)
    #   올라가는 계단  (20,3)  · 위쪽 복도 · 아래에서만 · 앞칸 (20,5)
    #     → 1F 의 내려가는 계단 앞칸 (23,23) 으로 올려보낸다
    # GAME_DESIGN 6-3 정본: M4 → M3. B1 은 1F 아래라 올라가는 계단만 있다.
    m.place(E, "stairs_up", 20, 3)
    m.portal("m3_basecamp_1f", 20, 4, 23, 23, name="stairs_to_1f")
    m.portal("m3_basecamp_1f", 21, 4, 23, 23, name="stairs_to_1f")
    m.trigger("spawn_default", 20, 5, kind="spawn")
...
def m7(TS_):
    ...
    # x=0 이 맵의 맨 왼쪽 칸이라 더 왼쪽에 둘 자리가 없습니다. 대신 칸
    # 자체를 반 칸 왼쪽으로 밀어, 타일 0 의 **왼쪽 절반까지 들어가야**
    # 넘어가게 합니다 — 끝에 닿는 느낌이 납니다.
    for y in range(4, 8):
        m.open(0, y)
        m.portal("m3_basecamp_1f", 0, y, 42, 20 + (y - 4),
                 name="to_basecamp_1f", nudge=-24)

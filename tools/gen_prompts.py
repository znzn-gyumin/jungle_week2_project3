...
# ─────────────────────────────────────────────────────────────
# 히로인 6인
#   campus  평상복 외형 — ①② 스틸에 그대로 들어간다
#   garden  ① D7 새벽 커넥트가든의 순간
#   classroom ② D9 교육장 403 심야의 순간
...
HEROINES = [
    dict(
        ...
        garden_face="슬픔", classroom_face="화남", ending_face="기본",
        ...
        classroom=block("""
            ...
            모니터 빛이 아래에서 얼굴을 비춤
        """),
        ending_age=27, ending_bg="winebar",
        # 표정 칸만으로는 안 된다. 기본 칸을 물렸더니 침울로 굴러갔다.
        # 칸은 얼굴을 정하고 시선과 맥락이 감정을 정한다 — 클라이맥스에서
        # 이미 두 번 확인했다. 주인공은 1인칭이라 화면 이쪽이 그 자리다.
        # 기쁨 칸으로 바꾸지 않는 이유: 민아의 기쁨은 눈 감고 입 벌려 웃는
        # 그림인데 이 컷은 특별할 것 없는 저녁의 익숙함이다. 열이틀 내내
        # 무표정이던 사람은 눈매만 풀려도 그게 더 큰 변화로 읽힌다.
        ending=block("""
            같은 회사 다른 팀, 여전히 코드 리뷰로 다투고 퇴근은 같이 함.
    ...
    dict(
        ...
        garden_face="부끄러움", classroom_face="슬픔", ending_face="기쁨",
        # 조원을 긍정에 적어야 그려진다. 부정에서 "여러 명, 군중" 을 뺀 건
        # 안 막았을 뿐이지 넣으라는 뜻이 아니다. 빈 강의실에 혼자 나왔다.
        #
        # 6조는 주인공 + 히로인 3 이다. 승희를 빼면 민아와 윤정 둘뿐이고,
        # 주인공은 목격자라 무리에 없다 (카메라 자리이고 1인칭이라 얼굴이
        # 화면에 안 나온다). 그래서 정확히 두 명이다.
        # 얼굴은 그리지 않되 실루엣 갈고리는 준다 — 노란 후드와 높게 묶은
        # 머리면 어둠 속에서도 누군지 읽힌다. 모르는 사람들이 결정한 게
        # 아니라 아는 둘이 빼놓은 것이어야 아프다.
        classroom=block("""
            뒤쪽 책상에 조원 두 명이 모여 무언가를 정하고 있음,
            한 명은 노란 오버사이즈 후드, 한 명은 검은 머리를 높게 묶음,
            둘 다 뒷모습이라 얼굴은 화면에 없음, 아무도 이쪽을 보지 않음,
            그 무리에서 두세 걸음 떨어져 혼자 서 있음,
            소매로 손을 감싼 채 말을 꺼내지 못하고 시선을 내림
        """),
        ending_age=28, ending_bg="gallery",
        # 기쁨 칸을 물렸더니 눈을 감고 입 벌려 웃는 그림이 나왔다. 승희는
        # 여섯 중 표정 변화가 가장 작은 인물이고, 무엇보다 눈을 감으면
        # 아무도 안 보는 그림이 된다. 말을 못 하던 사람의 엔딩에서 제일
        # 중요한 건 마주 보는 것이다.
        #
        # 전시 작품을 인물화로 정한다. 캠프 때 스케치북에 사람들 뒷모습만
        # 그리던 사람이라, 그게 벽에 걸려 있으면 "뺏기듯 보여주던 사람이
        # 이제 걸어놓는다" 가 성립한다. 가든 스틸과 엔딩이 한 줄로 이어진다.
        ending=block("""
            이제 먼저 연락하는 사람이 됨, 자기 전시 작품 옆에 서 있음.
    ...
    dict(
        ...
        """),
        garden_face="슬픔", classroom_face="놀람", ending_face="기쁨",
        # 놀람 칸만 물리면 "깜짝이야" 가 나온다. 이건 모욕을 들은 순간이라
        # 놀람과 상처가 섞이고 그 다음에 말문이 막혀야 한다.
        #
        # 말하는 쪽은 6조가 아니라 4조다. 6조는 주인공(1인칭이라 화면에 안
        # 나오고, 할 일이 "나이를 빼고 칭찬한다" 다)과 민아·승희뿐이라 그
        # 말을 할 사람이 없다. 4학년이 2학년에게 하는 말이라 강태윤·강태연의
        # 자리이고, 그 둘은 흰 셔츠에 남색 니트 조끼다.
        classroom=block("""
            헤드폰은 목에 걸려 있음,
            바로 앞에 4조(경쟁 조) 사람 한 명이 등을 보이고 서 있음,
            흰 셔츠 위에 남색 니트 조끼, 뒷모습만 보이고 얼굴은 화면에 없음,
            방금 들은 말에 눈이 커지고 입이 반쯤 벌어진 채 굳음,
            놀라움이 아니라 상처받은 얼굴, 하려던 말이 끊긴 채 소리가 안 나옴
        """),
        ending_age=26, ending_bg="gallery",
        # 시연용 책상에 노트북을 올려놓으니 미술관 한가운데 작업 자리가
        # 생겼다. 뒤에서 그림 보는 관람객들과 따로 논다. 게임이 미술관에
        # 있는 게 문제가 아니라 전시물이 아니라 시연대로 놓인 게 문제다.
        # 벽에 설치된 화면으로 바꾸면 앞뒤가 맞고, "졸업작품이 미술관에
        # 걸렸다" 가 되어 이야기도 커진다.
        # 승희와 같은 미술관인 건 자연스럽다 — 둘 다 그래픽디자인학과다.
        # 승희는 액자 인물화 앞, 윤정은 큰 화면 앞이라 하는 일이 갈린다.
        ending=block("""
            졸업작품이 출시된 게임이 됨, 그걸 보여주는 중.
            벽에 크게 설치된 화면에서 게임이 돌아가고,
            그 앞에 낮은 전시대와 컨트롤러가 붙박이로 설치되어 있음,
            관람객 한둘이 서서 직접 조작해보는 중, 옆에 전시 설명 패널,
            접이식 책상이나 노트북 같은 임시 시연대는 없음
        """),
        ending_wear=block("""
    ...
    dict(
        ...
        garden=block("""
            ...
            데크와 화단은 거의 검게 가라앉아 있음
        """),
        garden_face="기본", classroom_face="기본", ending_face="기본",
        classroom="화면 속 결과물은 완벽한데 표정은 완전히 비어 있음, 아무도 눈치채지 못함",
        ending_age=27, ending_bg="winebar",
        # 오피스일 이유가 없었다. "건강해짐" 은 일 밖의 삶이 생겼다는 뜻인데
        # 책상 앞에 앉혀두면 그게 안 보인다. 그리고 이 인물의 클라이맥스가
        # "아무도 눈치 못 챔" 이라, 5년 뒤의 변화는 누군가와 편하게 이야기하고
        # 있다는 것이다.
        # 민아와 같은 와인바지만 둘은 한 회차에 같이 안 나온다(여주/남주가
        # 갈린다). 동갑 자리의 거울상이고 모티프도 같아서 오히려 대칭이 된다.
        # 자세로 갈린다 — 민아는 앉아서 마주 보고, 민규는 창가에 걸터선다.
        ending=block("""
            이제 제때 자고 제때 먹음, 건강해짐, 차분함.
            퇴근하고 온 자리, 창턱에 걸터서서 한쪽 팔을 창틀에 걸치고 잔을 들고 있음,
            앉아 있지 않음, 창밖은 도시 야경,
            화면 이쪽을 향해 이야기하는 중, 말이 편하게 나오는 담담한 얼굴
        """),
        ending_wear=block("""
            회색 셔츠.
            **참조 이미지의 긴 앞머리를 쓰지 마세요** — 앞머리를 완전히 넘겨
            이마가 다 드러나 있음, 눈 밑에 다크서클이 없음,
            볼에 살이 조금 붙어 건강해 보임
        """),
    ...
    dict(
        ...
        garden_face="슬픔", classroom_face="슬픔", ending_face="기본",
        classroom="책상에 쓰러짐, 쾌활함이 마침내 벗겨짐",
        ending_age=29, ending_bg="office",
        # 넥타이를 매주는 사람이 얼굴까지 나왔다. 승민 루트는 여주 플레이라
        # 그 사람이 주인공인데, 주인공은 1인칭이라 얼굴이 화면에 나오면 안 된다.
        # 손만 보이게 하면 플레이어가 곧 그 손이 되어 오히려 낫다.
        ending=block("""
            새 직장 첫 출근날, 이번엔 누군가 그를 챙겨줌.
            화면 이쪽에서 뻗어 나온 두 손이 넥타이를 매주고 있음,
            그 사람의 얼굴과 몸은 화면에 없고 손만 보임,
            쑥스러워하면서도 가만히 서 있음, 긴장했지만 불안하지는 않은 얼굴
        """),
        ending_wear=block("""
    ...
    dict(
        ...
        garden=block("""
            ...
            데크와 화단은 거의 검게 가라앉아 있음
        """),
        garden_face="슬픔", classroom_face="슬픔", ending_face="기쁨",
        # 뱃지는 세 씬 전부에 "딱 하나" 를 박아야 한다. 한 번만 적으면 모델이
        # 랜야드 옆에 하나를 더 만든다. 윤정 헤드폰과 같은 현상이다.
        # 시선 회피도 말로 적어야 한다 — 안 적으면 카메라를 본다.
        classroom=block("""
            뭘 하고 싶냐는 질문을 방금 받음, 대답하지 못함,
            시선을 옆 아래로 돌려 화면 이쪽을 보지 않음, 카메라와 눈을 맞추지 않음,
            입을 열려다 만 채로 멈춤.
            강아지 뱃지는 가슴에 딱 하나뿐, 명찰이나 랜야드에는 붙이지 않음
        """),
...
BG_ENDING = {
    ...
    # 민아 전용. 여섯 중 "퇴근은 같이 함" 이 명시된 유일한 인물이라
    # 사무실이 아니라 그 다음 자리가 필요하다.
    "winebar": block("""
...
def main() -> int:
    ...
    for h in HEROINES:
        ...
        write(OUT / "04_still" / f"T{idx:02d}_{h['id']}_ending.md", prompt_file(
            ...
            attach, prompt,
            NEG_BASE + "\n" + (NEG_ENDING_EXTRA.replace(NEG_NIGHT, "10대 같은 인상,")
                               if bgk == "winebar" else NEG_ENDING_EXTRA).rstrip(","),
            h["ending_note"] + "\n\n"
            "> **엔딩에서 랜야드를 통짜로 막으면 안 됩니다.** 차단 대상은 **캠프 명찰**이지 "
            "사원증이 아닙니다 — 민아는 사원증 랜야드를 걸고 윤호는 강아지 뱃지가 "
            "사원증 스트랩에 달립니다.\n\n"
            + ("> **이 컷만 밤입니다.** 다섯은 낮·실내라 `밤 장면, 어둠`을 부정에 넣지만 "
                "민아만 뺐습니다 — 퇴근하고 온 자리라 밤이어야 합니다."
                if bgk == "winebar" else

...
| **반신은 한 번 서면 씬이 끝날 때까지 있습니다** | 세우는 것은 첫 대사입니다 — 나레이션으로 시작하는 CG 컷의 첫 몇 줄은 배경만 남습니다. 그다음부터는 **나레이션과 주인공 대사에서도 안 내립니다.** 명장면은 나레이션이 절반이 넘어서(커넥트가든 씬은 35줄 중 20줄), 말할 때만 세우면 인물이 두 줄 보이고 세 줄 사라지기를 반복합니다 |
| **에필로그에는 반신이 서지 않습니다** | 5년 후는 두 벌 중 어느 쪽도 아닙니다 — 엔딩마다 새 옷을 내레이션이 따로 묘사하고(「검정인데 후드가 아니다」) 그 모습은 엔딩 스틸이 품고 있습니다. **결말 씬은 첫 줄부터 그 스틸 한 장으로 갑니다** — `@bg` 바로 뒤에 `@cg` 를 걸어 두고 끝까지 안 내립니다. 배경은 스틸 뒤에 깔린 채로 Normal 6종과 솔로에만 보입니다(그 셋은 스틸이 없습니다) |
| **`@bg` 는 캠퍼스를 떠난다는 뜻입니다** | `@map` 이 기억해 둔 장소를 여기서 놓습니다. 안 놓으면 바로 다음 `@char` 가 그 장소 사진으로 배경을 덮어씁니다 — 외출 세 곳과 귀소 버스가 통째로 커넥트가든 사진에 가려져 있었고, 그 사진이 캠퍼스 벌이라 반신까지 명찰 찬 후드로 서 있었습니다 |
...
| **스틸은 씬이 끝날 때까지 남습니다** | `@cg`를 건 씬은 그 스틸로 끝냅니다. 몇 줄 뒤 반신으로 되돌리면 **한 장면 안에서 그림체가 두 번 갈립니다** — 명장면이 스틸을 지목하고 있으므로 그 장면의 그림은 스틸입니다. 내리려면 `@cg none`을 쓸 수 있지만 지금 대본에는 한 줄도 없고, 씬이 바뀌면 자동으로 내려갑니다 |
| `@time 시간대` | **이 장면이 몇 시인가** — `day` `evening` `night` `deepnight` `dawn`. 평소에는 `@bgm`이 시각을 대신하므로 안 적습니다. **곡은 감정을 따라가고 시각은 안 따라가서** 어긋나는 자리에만 씁니다 — `swell`은 새벽 두 시와 수료식 낮에, `tension`은 저녁 코리와 새벽 밤샘에 같이 쓰입니다. 배경 사진과 타일맵의 색조가 이 값을 봅니다([assets/README 배경 보정](../assets/README.md#배경-보정)). 씬이 바뀌면 풀리고, 라벨 점프에서는 유지됩니다 — 커넥트가든 씬이 심야로 시작해 `--- dawn ---`에서 여명으로 넘어가는 자리가 그것입니다 |
| `* 텍스트` | 내레이션 |
...
- **`displayName`은 화면에 안 나옵니다.** 방명록에 적히는 이름은 게임에서 정한 이름(`who`)입니다 — 12일을 그 이름으로 불린 사람이 남기는 글이라 실명 계정이 뜨면 세계가 깨집니다. 계정 이름은 규칙이 토큰과 대조해 **사칭을 막는 자리**로만 씁니다
...
```
users/{uid}
  ├─ profile          { displayName, firstSeenAt, clearedRoutes: RouteId[], seenCG: string[],
  │                     clears: { [엔딩]: number }, maxDay: number }
...
- **엔딩은 스틸이 뜰 때가 아니라 끝까지 봤을 때 실립니다** (`claimEnding`). 「처음인가」를 물으려면 적어 넣기 **전에** 세어야 하고, 그림이 스친 시점보다 끝까지 본 시점이 수집한 때입니다. `clears`는 도감과 달리 **다시 봐도 올라갑니다** — 마지막 화면의 「도감이 해금되었습니다 / 클리어 카운트가 올라갑니다」가 이 둘을 가릅니다
- **도감은 루트 엔딩 여섯 장입니다.** True와 Good이 같은 스틸을 쓰므로 한 루트에 한 자리입니다. **Normal 6종과 솔로는 안 실립니다** — `@cg`가 없어서, 넣으면 플레이어가 본 적 없는 그림이 열립니다. 그 일곱은 횟수만 씬 이름으로 셉니다
- **스틸이 없는 일곱(Normal 6 + 솔로)은 마지막 한 줄이 다릅니다** — 「오늘도 외로운 하루입니다」. 배경도 `night_window` 한 장을 같이 쓰는 씬들이고, 끝내 말을 못 걸고 돌아 나온 결말이라 수집이나 횟수를 알리는 자리가 아닙니다. 가르는 기준은 씬 이름이 아니라 **도감에 자리가 있는가**(`Claim.inBook`)입니다
- **솔로 엔딩은 지금 대본으로 도달할 수 없습니다** — `p_night`의 선택지 여섯이 전부 루트를 정하므로 `route`가 비는 경우가 없습니다

| 엔딩 | 첫 클리어 | 그 이후 |
|---|---|---|
| True · Good (루트 6 × 2) | 도감이 해금되었습니다 | 클리어 카운트가 올라갑니다 |
| Normal 6 · 솔로 | 오늘도 외로운 하루입니다 | 오늘도 외로운 하루입니다 |

같은 루트의 True와 Good은 **스틸이 같아서 한 자리를 나눠 씁니다.** 먼저 본 쪽이 「해금」을 가져가고 나중 쪽은 「카운트」입니다 — 티어 순서와 무관합니다
...
guestbook/{uid}          // 문서 ID = 작성자 uid → 1인 1글이 구조로 강제됨
  { uid,
    who,                     // **화면에 적히는 이름** — 게임에서 정한 성+이름 (12자)
    displayName,             // 구글 계정 이름. 화면에는 안 쓰고 사칭 차단에만 씀

...
**DOM 배경은 화면을 1920 × 1080으로 채우고, 사진 배경에만 CSS 필터로 회화풍 보정을 겁니다** — 값은 [`assets/README` 배경 보정](../assets/README.md#배경-보정)에 있습니다. 타일맵 조명([5-3](#5-3-챕터별-상태))과 별개 경로입니다. Phaser는 캔버스에 오버레이를 얹고, DOM은 CSS로 처리합니다.
...
| `@char 이름 위치` | 캐릭터 입장 (left/center/right). **히로인 6인 전용**입니다 — 반신 CG가 그 여섯에게만 있습니다([GAME_DESIGN 6-2](./GAME_DESIGN.md#6-2-이벤트-cg-히로인-전용)). 조연 다섯은 도트 스프라이트뿐이라 **대사창 이름만으로 말합니다** |
...
| **에필로그에는 반신이 서지 않습니다** | 5년 후는 두 벌 중 어느 쪽도 아닙니다 — 엔딩마다 새 옷을 내레이션이 따로 묘사하고 그 모습은 엔딩 스틸이 품고 있습니다. 그래서 결말 씬은 `@char` 없이 **배경 + 대사창**으로 가고 스틸이 마지막 그림이 됩니다 ([assets/README 배경 보정](../assets/README.md#배경-보정)) |
| `@char * 위치` | **현재 루트의 히로인을 세웁니다** — `-> r_*_` 와 같은 규칙입니다. 공용 씬은 히로인 이름을 적을 수 없는데, 귀소 버스가 이걸 필요로 합니다: [배경 보정 3겹](../assets/README.md#배경-보정)이 `bus_night` 씬의 반신 CG 를 지목하고 있어서 반신이 서야 그 겹이 성립합니다 |
...
| `이름 [표정] "대사"` | 대사 한 줄. 표정 생략 시 직전 표정 유지. 표정은 **여섯뿐**입니다 — `기본` `기쁨` `부끄러움` `슬픔` `놀람` `화남`. **파일명은 영문**이라 컴파일러가 [대응표](../assets/README.md#id)로 바꿉니다 |

...
> **배경 9장과 UI**입니다. 캐릭터는 [character/ART_BRIEF.md](./character/ART_BRIEF.md), 타일셋·타일맵은 [TILE_BRIEF.md](./TILE_BRIEF.md)에 따로 있습니다.
>
...
| ⑧ | D12 수료식 | **사진** — 캠프 기간 |
| ⑤ ⑤b ⑥ ⑦ | 오피스 · **와인바(밤)** · 전시장 · 밤 창가 | **일러스트** — 5년 후 |
...
> **인물을 넣지 않습니다.** 대화 중 뒤에 깔리는 배경이라 그 위에 반신 CG가 올라갑니다. 다만 이 엔딩들은 **화면에 아무도 안 나오는 게 정서**라, 실제로는 배경만 보이는 시간이 깁니다.

#### 프롬프트 — 아래 상자를 통째로 복사하세요

```text
가로 16:9 와이드 구도, 1920 x 1080. 배경만, 인물 없음.

밤의 어두운 방, 큰 창가에 놓인 책상,
켜져 있는 모니터 하나가 방 안의 유일한 광원,
유리창 너머 저 아래로 도시 불빛이 멀리 희미하게 보임,
방의 나머지는 깊은 그림자에 잠겨 있음,
책상에 마시다 만 머그컵 하나가 놓여 있음,
혼자 있는 방의 조용함.

한국 웹툰 배경 화풍, 세미리얼 애니메이션 배경화, 부드러운 회화적 렌더링,
깔끔한 구도, 비주얼 노벨 배경.

마지막으로, 다음은 절대 넣지 마세요 — 인물, 사람, 앞쪽에 서 있는 사람,
클로즈업된 얼굴, 글자, 워터마크, 서명, UI 요소, HUD, 프레임 테두리,
사진 같은 실사, 3D 렌더, 어안 렌즈, 기울어진 수평선,
밝은 실내, 천장 조명이 켜진 방, 여러 개의 광원.
```

#### 받고 나서 확인할 것

- [ ] **광원이 모니터 하나뿐인가** — 천장 조명이 켜져 있으면 다시
- [ ] 방의 나머지가 **읽히지 않을 만큼 어두운가**
- [ ] 창밖 도시 불빛이 **멀고 희미한가** (가까우면 방이 밝아 보입니다)
- [ ] **사람이 없는가**
- [ ] 1920 × 1080 가로인가
...
1. **일러스트 배경 9장** — 생성에 가장 잘 맞고 다른 작업에 안 막힙니다. 지금 바로 시작 가능

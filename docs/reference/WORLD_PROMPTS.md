...
> **캐릭터를 뺀 나머지 전부**입니다. 캐릭터는 [reference/character/ART_BRIEF.md](./character/ART_BRIEF.md)에 따로 있습니다.
>
...
| 항목 | 수량 | 생성 도구로 되는가 |
|---|---|---|
| **① 일러스트 배경** | **8장** | ✅ **가장 잘 맞습니다.** 그대로 뽑으면 됩니다 |
| **② 타일셋** | 3종 | ⚠️ 조각은 뽑히지만 **이어붙는 타일로는 안 나옵니다.** 정리가 필요합니다 |
| **③ 도트 타일맵** | 7개 | ❌ **생성 불가.** Tiled에서 타일셋으로 조립합니다 |
| **④ UI 화면** | 1종(+3 참고) | ✅ **HTML/CSS 도구에 잘 맞습니다** |
| **⑤ 조명 오버레이** | 5종 | — 그림이 아니라 **색값**입니다 (6절) |
...
**규격**: 일러스트 배경은 **1920 × 1080 가로, 불투명**입니다.

---

## 2. 일러스트 배경 — 8장

### ① 한국민속촌 — D7 외출 (승희 · 윤호 선호)
...
### ⑤ 5년 후 · 오피스 — True/Good (민아 · 민규 · 승민 · 윤호)
...
### ⑦ 5년 후 · 밤 창가와 모니터 불빛 — **Normal 6종 + 솔로 공통**
...
| 타일셋 | 쓰이는 맵 |
|---|---|
| **교육동 실내** | M1 교육동 4F · M2 2F · M3 1F · M4 B1 |
| **숙소동 실내** | M6 숙소동 |
| **야외** | M5 커넥트가든 · M7 정문 |

**규격**: 타일 1칸 **48 × 48px**, 투명 배경 PNG, 격자에 정확히 맞을 것.

### 생성 도구의 한계

**이어붙는 타일셋은 생성으로 안 나옵니다.** 격자가 어긋나고 인접 타일의 이음매가 안 맞습니다. 실제 경로는 이렇습니다.

1. 생성으로 **분위기 시안**을 뽑습니다 (색감·질감·디테일 밀도 확인용)
2. 그 시안을 보면서 **48px 격자 위에 직접 찍습니다**
3. 바닥 타일은 **상하좌우가 이어지는지** 반드시 실제로 붙여 확인합니다

시안용 프롬프트:

```
top-down oblique pixel art tileset for a 2d rpg, 48px tiles, [아래 내용],
limited palette, clean 1px dark outlines, retro jrpg style, tile grid layout
```

| 타일셋 | `[내용]` |
|---|---|
| **교육동 실내** | `office carpet floor, white walls, glass whiteboards, long desks with monitors, office chairs, ceiling light panels, floor-to-ceiling windows with curtains, stairwell, vending machine` |
| **숙소동 실내** | `dormitory corridor, room doors with number plates, twin beds, desks, laundry machines, gym equipment, stair landing` |
| **야외** | `wooden deck planks, stone planter walls, young trees, outdoor tables and chairs, paved path, grass patches, bollard lights, bus stop, parking lot lines` |

**화풍 기준은 히로인 도트입니다.** 타일셋을 캐릭터보다 먼저 확정하면 안 됩니다 — 스프라이트 정면 1장과 타일 샘플 한 조각을 **함께** 확정하고 시작하세요.

---

## 4. 도트 타일맵 — 7개 (생성 불가)

**Tiled 에디터에서 타일셋으로 조립합니다.** 생성 도구가 할 일이 없습니다.

| # | 맵 | 들어가는 공간 | 주로 나오는 시간대 |
|---|---|---|---|
| **M1** | 교육동 4F | **교육장 403(지정석)** · 코칭실 403 · 커뮤니티 라운지 · 복도 | 낮 / 저녁 / 밤 / 심야 |
| **M2** | 교육동 2F | 오픈데스크 존 · 회의실 5~7 · 라운지 | 낮 / 심야 |
| **M3** | 교육동 1F | 로비 · 정글라운지 · 운영사무실 · 택배보관소 | 낮 / 밤 |
| **M4** | 교육동 B1 | 정글스테이지 · 카페테리아 · 편의점 · 그랩앤고 | 낮 / 심야 |
| **M5** | **커넥트가든** | 카페 앞 데크 · 통로 | 밤 / 심야 / 여명 |
| **M6** | 숙소동 | 1F 체력단련실·세탁실 · 3F 남 / 4F 여 복도 · 2인실 · **3F↔4F 계단참** | 밤 |
| **M7** | 정문·버스정류장 | 주차장 · 로터리 | 낮 / 밤 |

- 레이어: `ground` / `objects` / `collision` / `npc`(오브젝트) / `portal`(오브젝트)
- 파일명: `m1_basecamp_4f.json` `m2_basecamp_2f.json` `m3_basecamp_1f.json` `m4_basecamp_b1.json` `m5_connect_garden.json` `m6_nestcamp.json` `m7_gate.json`
- **M1이 허브, M5가 감정 정점**입니다. 이 둘만 먼저 만들어도 게임이 돕니다
- 교육동 3F는 이번 기수가 안 쓰는 빈 층이라 맵을 만들지 않고 계단을 4F↔2F 직결로 처리합니다

**공간 배치의 근거는 캠퍼스 사진**입니다. 낮 사진에서 밤 구조를 읽기 어려우면 [9~12절](#9-캠퍼스-사진-시간대-변환--규칙)로 시간대 레퍼런스를 먼저 뽑으세요.

### M1에 반드시 들어가야 하는 것

**화이트보드**입니다. 인게임 낙서가 쌓이는 곳이자 **플레이어 방명록이 얹히는 자리**라, 상호작용 지점으로 잡아야 합니다. 교육장 403 **앞쪽**에 있고 **맞은편이 통창**입니다.
...
## 6. 조명 오버레이 — 5종 (그림이 아닙니다)

타일맵은 **맵당 한 벌**이고, 시간대는 그 위에 오버레이를 얹어 만듭니다. 아래는 시작값이고 실제 타일맵에 얹어보며 조정하세요.
...
| **여명** | `#5B3E8C` → `#3E6BB0` 그라데이션 | screen | ~40% | M5 전용 |

**심야를 72%까지 깊게 두는 게 이 게임의 정체성입니다.** 주변에 도시 불빛이 없어 얼굴이 거의 안 보이는 게 기본값이고, 그 어둠 때문에 여명 전환이 강해집니다.
...
- [ ] 바닥 타일을 4장 붙였을 때 **이음매가 안 보이는가**
- [ ] 캐릭터 도트와 **색 수·외곽선 규칙이 같은가**
...
3. **타일 샘플 한 조각** — 히로인 도트 정면 1장이 나온 뒤에. 둘을 나란히 놓고 화풍을 함께 확정
4. **타일셋 3종 → 타일맵 M1 · M5** — 이 둘만 있으면 게임이 처음부터 끝까지 돕니다
5. 나머지 타일맵 5개
...
1. **타일맵 그릴 때** 옆에 띄워두고 어디에 빛이 남는지 확인
2. **스틸 CG 2종**(커넥트가든 여명 · 403 심야)의 배경 구도 잡기
3. **오버레이 5종 수치 검증** — 생성 결과의 색을 스포이드로 찍어 오버레이 값과 대조

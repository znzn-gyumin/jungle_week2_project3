...
1. **화풍 밀도** — 판단 보류 중입니다. 타일은 외곽선이 옅은 저채도 미니멀에 27~32색이고,
   캐릭터는 1px 외곽선에 8~12색입니다 ([SPRITE_SPEC 1절](./character/SPRITE_SPEC.md#1-규격)).
...
## 10. 2차 수정 이력 (NEXT_TILE.md 대응)

| 항목 | 무엇을 고쳤나 |
|---|---|
| ① M1 좌석 | 균일 4행×2블록 그리드 → 줄마다 pod 크기가 다른 배치. 동갑·연상·연하를 주인공 기준 세 방향으로 분산, 4조는 간격 2칸 밀착. 25석 → **24석**(주인공 포함), 무명 17 확정 |
| ② M5 의자 | 테이블 8세트 + 초록 하이테이블 2조, 앉을 자리 27개. `high_table` `stool_g/w` `chair_out_left/right` 신규 타일 4종. `two_chairs` 트리거 추가 |

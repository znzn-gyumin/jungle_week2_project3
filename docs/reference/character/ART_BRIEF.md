...
**48px에서는 실루엣과 색이 전부입니다.** 열둘에 멀리서도 읽히는 갈고리를 하나씩 줬습니다.

| 인물 | 도트 외형 | 멀리서 읽히는 것 |
|---|---|---|
| **이도윤** 남 22<br>**이도아** 여 22 | 회색 후드집업 · 검은 하의 · 흰 운동화. 도윤은 짧고 정돈 안 된 머리, 도아는 어깨 길이를 하나로 묶음 | **무채색.** 여섯 중 누구와 서 있어도 배경이 되는 게 주인공의 자리입니다 — 학번도 실력도 가운데 |
| **한지오** 남 22<br>**한지아** 여 22 | **코랄 티셔츠** · 반바지 · 슬리퍼. 지오는 부스스한 머리, 지아는 대충 묶은 똥머리 | **겉옷이 없는 유일한 인물.** 팔이 드러납니다 — 히로인 여섯은 냉방 때문에 겉옷을 걸치는데 절친만 반팔입니다. 같은 방을 쓰는 사이라 격식이 제일 없습니다 |
| **명진혁** 남 32 | **검은 셔츠를 팔꿈치까지 걷어올림** · 검은 슬랙스 · **검은 구두**. 전신 무채색. 랜야드가 **운영진용 다른 색** | **혼자 셔츠와 구두.** 학생들 사이에서 유일한 어른 옷 |
| **조민** 21 (중성적) | 짙은 회색 후드를 **뒤집어쓰고** 있음 · 검은 팬츠. 성별이 안 읽히게 | **후드를 쓴 유일한 인물.** 얼굴이 늘 그늘 |
| **강태윤** 남 23<br>**강태연** 여 23 | 흰 셔츠·블라우스 위에 **남색 니트 조끼** · 베이지 치노/슬랙스 · 흰 스니커즈 | **가장 단정.** 승민이 "갖춰 입은 학생"이면 이쪽은 **이미 취업한 사람처럼** 보입니다 |
| **여사님** 여 50대 | **분홍 앞치마** · 흰 위생모 · 편한 셔츠 | **유일한 앞치마와 모자.** B1에서만 보입니다 |

### 공용 스프라이트 3종 — 무명 17명용

**한 종을 색만 바꿔 대여섯 번 재활용**합니다. 얼굴 디테일을 넣지 마세요 — 배경 인원이라 눈에 띄면 안 됩니다.
...
셋 다 **명찰 착용**, 머리는 단순한 덩어리로. 교체용 색은 이름 있는 인물이 쓰지 않는 색으로 고르세요 — **회갈 · 연보라 · 올리브 · 남색 · 벽돌** 정도.
...
**같은 이미지 안에 있으면 같은 사람입니다.** 낱장으로 6번 뽑으면 얼굴이 매번 달라지는데, 시트는 구조적으로 그 문제가 없습니다. [`pixel_demo.png`](./pixel_demo.png)가 그 증거입니다 — 3인 × 22컷이 한 장에서 흔들림 없이 나왔습니다.
...
| **도트 288컷** | **시트 18장** | 시트가 가장 잘 듣는 자리 ([14절](#14-도트-시트--18장--288컷)) |
...
**시트가 가장 잘 듣는 자리입니다.** 셀을 48px로 **줄이는** 작업이라 확대할 때와 반대로 픽셀이 선명해집니다.
...
주인공 2 · 절친 2 · 조연 5 · 공용 3은 프로필이 없습니다. **히로인 도트 한 종을 먼저 확정**하고 그걸 참조 이미지로 물려서 뽑습니다. 인물 설명은 [5절](#참조-이미지가-없는-12종).
...
3. **김민아 도트 시트 1장** → 48px로 줄여 `pixel_demo.png`와 비교. **전체 화풍 확정**

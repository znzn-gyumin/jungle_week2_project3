...
| **참조 이미지** | **김민규 평상복 몸통 확정본** |
| **선행 조건** | B07 확정 |
| **자르기** | `python tools/cut_sheet.py sheet_mingyu_face.png out/mingyu --cols 3 --rows 2 --dekey 00b140 --tol 40 --trim --names normal,happy,shy,sad,surprise,angry` |

> **시선을 안 주는 게 기본 표정**입니다. 웃는 셀에서도 눈이 화면 밖을 봅니다.
...
avoids eye contact even when smiling
```

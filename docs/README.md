...
**설계·에셋·대사가 다 닫혔고 구현이 남았습니다.** 순서는 [TECH_DESIGN 7절](./TECH_DESIGN.md#7-구현-순서)이 정본이고, **7·9번(스크립트 작성)은 이미 끝났습니다.**

1 · 3~6 · 8 · 10~11번까지 됐습니다 — 스캐폴딩·배포 · `.vns` 파서 · VN 재생기 · 배경/CG · Phaser 맵 · **로그인·세이브·방명록**. 남은 것은 **2번 Firebase 프로젝트 생성**뿐인데 이건 콘솔 작업이라 직접 해야 합니다. **BGM 8곡**은 여전히 없습니다.

**코드는 다 붙어 있고 설정만 비어 있습니다.** 저장소 루트의 `.env.example` 을 `.env` 로 복사해 웹 앱 config 를 채우면 로그인·세이브·방명록이 그 자리에서 살아납니다. 값이 비어 있는 동안에는 그 셋만 잠기고 게임 본편은 그대로 돕니다 — 프로젝트를 만들기 전에도 시나리오와 맵을 손볼 수 있어야 하기 때문입니다. 보안 규칙은 `firestore.rules` 에 있고 콘솔에 그대로 붙여 넣으면 됩니다.
